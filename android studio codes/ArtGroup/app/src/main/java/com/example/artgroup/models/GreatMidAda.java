...
public class GreatMidAda extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<GreatMes> list;
    private Context ctx;

    public GreatMidAda(Context context, List<GreatMes> list) {
        this.list = list;
        ctx = context;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder hotel, int i) {
        if (hotel instanceof OriginalViewHolder) {
            ...
            if (get.getMeme().equals("M")) {
                ...
                viewHolder.sName.setText(get.getName());
