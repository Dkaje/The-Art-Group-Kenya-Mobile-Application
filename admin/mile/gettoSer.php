...
include('../database/conn.php');
    $custid = $_POST["custid"];
    $status = $_POST["status"];
    $response = mysqli_query($con,"SELECT * FROM ser_pay where custid='$custid' and status='$status' order by reg_date desc");
    if (mysqli_num_rows($response) > 0) {
        $results['trust'] = 1;
        $results['victory'] = array();
        while ($row = mysqli_fetch_array($response)) {
            $index['serid'] = $row['serid'];
            ...
            if ($row['disb'] == 0) {
                ...
                $index['disb'] = 'Driver & Videographer Attached';
            }
